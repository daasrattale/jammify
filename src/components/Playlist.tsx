...
export const Playlist = ({ playlist, setPlaylist }: PlaylistProps) => {
     ...
     return (
          <div className="m-5 p-5 border rounded-xl border-gray-700 bg-black/70">
               <div className="mb-7">
                    {
                         playlist.title == null || isModifying
               ...
               <div className="text-center py-4">
                    <button onClick={isModifying ? saveTitle : savePlaylist} disabled={playlist.tracks.length == 0 && !isModifying} className="bg-purple-500 px-4 py-2 rounded-full disabled:bg-gray-600 disabled:text-gray-500">{isModifying ? 'Save title' : 'Save to Spotify'}</button>
               </div>
          </div>
     );
}